...
- Continue to track the Codex `session_id` so conversation context resumes, not just
  the filesystem.
- Preserve all v0.1.1 product behavior, persistence guarantees, contracts, API
  endpoints, and UI.
...
- No change to the Postgres schema, API endpoints, shared contracts, or UI product
  model.
...
- Product model, persistence, API, and UI are exactly as shipped in v0.1.1.
...
- Shared contracts in `packages/shared/src/chat.ts`, including `CreateAgentTaskRequest`,
  `RunnerAgentTaskResponse`, and the `sessionId` / `workDir` fields.
- Web UI.
...
- `execInSandbox(sandbox, command, { apiKey, apiEndpoint, timeoutMs })` — runs the
  Codex command inside the sandbox, injecting the API key and endpoint as environment
  variables for that execution only, reads the final Markdown file, and collects raw
  output and events.
...
- **Kill on archive:** when a `chat_session` transitions to `archived`, its sandbox is
  destroyed.
...
v0.1.2 baseline is **Option C**: smallest change, adequate for the demo, and naturally
drop-resilient. Option A is recorded as a later upgrade.
...
- The API key is injected into the sandbox as an environment variable only for the
...
- Unchanged v0.1.1 API tests continue to pass, since the contract and endpoints are
  unchanged.
...
- Sandbox lifecycle (pause on completion, kill on archive) and cost expectations.
...
6. Lifecycle: pause on completion, kill on archive, resume-failure fallback.
